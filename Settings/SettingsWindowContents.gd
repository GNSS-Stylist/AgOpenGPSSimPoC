...
func _on_button_update_udp_pressed():
	var subnet:Array[int] = [
		int($UDP/VBoxContainer/GridContainer/SpinBox_IP_1.value),
		int($UDP/VBoxContainer/GridContainer/SpinBox_IP_2.value),
		int($UDP/VBoxContainer/GridContainer/SpinBox_IP_3.value)
		] 
	
	get_parent().get_parent().updateUDPSubnet(subnet, $UDP/VBoxContainer/CheckBox_UseIP126ForServer.button_pressed)
